...
import ManJson from "../assets/json/man.json";
        ...
        <div className="bg-[#E1D9D1] dark:bg-[#36454F] dark:text-white">
          ...
          <div className=" flex flex-wrap gap-6   justify-center pt-[4rem] ">
            {ManJson.map((items, index) => (
              <div
                key={index}
                className="hover:scale-105  pb-[2rem] shadow-md hover:shadow-none text-center"
              >
                <img
                  src={items.image}
                  className="h-[17rem] w-[10rem] object-cover "
                />
                <h1>{items.Title}</h1>
